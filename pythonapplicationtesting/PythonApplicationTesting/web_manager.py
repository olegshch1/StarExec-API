...
# in progress
    ...
    def create_job(self, sid, name, desc, traversal, solver_id, bench_list, configs=671423, run_choice='choose', bench_choice='runChosenFromSpace',
        ...
        return self.session.cookies.get_dict()['New_ID']
#in progress
    def download_job_xml(self, id):
        paramlist = {'type':'jobXML', 'id':id}      
        response = self.session.get(self.preffix_url + 'download', params = paramlist, stream = True)
        response.raise_for_status()
        with open(f'download_folder\jobXML_{id}.zip', 'wb') as f:
            for chunk in response.iter_content(chunk_size=24000):
                if chunk:
                    f.write(chunk)
                    f.flush()
                    os.fsync(f.fileno())
